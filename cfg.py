...
class ContextFreeGrammar:
	...
	def __init__(self, grammar: str = None):
		...
		self.terminals.discard(EPS)
		self.terminals.discard(UNDERFLOW)
...
class PushdownAutomata:
	...
	def __init__(self, grammar: ContextFreeGrammar):
		...
		self.transition(START, 'Q0', EPS, PUSH, UNDERFLOW)
		self.transition('Q0', HUB, EPS, PUSH, grammar.startName)
		...
		for name, prodlist in grammar.productions.items():
			for prod in prodlist:
				...
				cur = HUB
				next = HUB if len(prod.terms) == 1 and terminal else self.names.get(name)
				if not terminal:
				...
				for i, term in enumerate(rest):
					...
					cur = next
					next = HUB if i == len(rest) - 1 else self.names.get(name)
		...
		for x in self.stateNames:
			for y in self.stateNames:
				for z in self.stateNames:
					...
					if all([x in main for x in [path, begin, end]]) and self.pathExists(x, y, z):
						if x == z:
							cfg.production(path, [])
						elif x != y and y != z:
